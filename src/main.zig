...
pub fn main() !void {
    ...
    if (std.mem.eql(u8, command, "info")) {
        ...
        try cmdDownload(allocator, source, output_dir, port, want_nostr);
        ...
        try cmdSeed(allocator, args[2], args[3], port, want_nostr, external_ip, description);
...
fn printUsage() void {
    ...
    stderr.print(
        ...
        \\  download <source> [--output-dir d] [--port p] [--nostr]
        \\           source: file.torrent, magnet:?..., or http(s):// URL
        \\           --nostr: also subscribe to nostr peer-announce events
        \\  seed <file.torrent> <data-dir> [--port p] [--nostr] [--external-ip <ip>] [--description "..."]
...
fn cmdDownload(allocator: std.mem.Allocator, source: []const u8, output_dir: []const u8, port: u16, want_nostr: bool) !void {
    if (std.mem.startsWith(u8, source, "magnet:")) {
        ...
        defer mi.deinit(allocator);
        startDownload(allocator, mi, output_dir, port, want_nostr);
    } else {
        // File path
...
fn startDownload(allocator: std.mem.Allocator, mi: carl.metainfo.Metainfo, output_dir: []const u8, port: u16, want_nostr: bool) void {
    std.fs.cwd().makePath(output_dir) catch {};
    var session = carl.session.Session.init(allocator, mi, output_dir, .download, port) catch |err| {
        log.err("failed to initialize session: {}", .{err});
        std.process.exit(1);
