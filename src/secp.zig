...
const c = @cImport({
    ...
    // libsecp256k1 marks its public API as __declspec(dllimport) on MSVC/MinGW
    // unless SECP256K1_STATIC is defined. We always link it as a static lib via
    // build.zig, so the import-style declarations would cause Windows builds
    // to fail looking for `__imp_*` symbols. Set the macro defensively even on
    // POSIX (it's a no-op there) so the @cImport is portable.
